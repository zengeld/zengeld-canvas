...
impl Primitive for Ellipse {
    ...
    fn points(&self) -> Vec<(f64, f64)> {
        vec![
            ...
            (
                self.center_bar + self.radius_bar,
    ...
    fn text_anchor(&self, ctx: &dyn RenderContext) -> Option<TextAnchor> {
        ...
        let cx = ctx.bar_to_x(self.center_bar);
        let cy = ctx.price_to_y(self.center_price);
        let rx = (ctx.bar_to_x(self.center_bar + self.radius_bar) - cx).abs();
        let ry = (cy - ctx.price_to_y(self.center_price + self.radius_price)).abs();

        // h_align: Start=left edge, Center=center, End=right edge
        let x = match text.h_align {
            TextAlign::Start => cx - rx + 10.0,
            TextAlign::Center => cx,
            TextAlign::End => cx + rx - 10.0,
        };

        // v_align: Start=top edge, Center=center, End=bottom edge
        let y = match text.v_align {
            TextAlign::Start => cy - ry + 10.0 + text.font_size / 2.0,
            TextAlign::Center => cy,
            TextAlign::End => cy + ry - 10.0 - text.font_size / 2.0,
        };
